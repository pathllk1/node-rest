...
export function openPartyModal(state, callbacks) {
    ...
    const renderPartyList = (data) => {
        ...
        data.forEach(party => {
            ...
            if (partyDiv) {
                fetch(`/api/inventory/purchase/party-balance/${partyId}`, {
                    method: 'GET',
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (balanceEl) {
                            const balanceColor = balanceData.balance_type === 'Credit' ? 'text-red-600' : balanceData.balance_type === 'Debit' ? 'text-green-600' : 'text-gray-600';
                            const balanceSymbol = balanceData.balance_type === 'Credit' ? '↑' : balanceData.balance_type === 'Debit' ? '↓' : '—';
